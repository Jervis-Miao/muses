...
/**
 * 多值比较校验器，支持Number、Date、String型比较 提供比较模式{flag}有：相等（EQ）、不相等（NE）、小于（LT）、小于等于（LE）、大于（GT）、大于等于（GE）
...
public class CompareValidator extends AbstractValidator {
    ...
    /**
     * 1、参与比较的值如果存在null值仅能参与EQ模式或NE模式比较 2、EQ模式下先进行快速失败比较（即相邻两个值比较） 3、NE模式下除进行快速识别比较，还需要比对所有值不能相等
