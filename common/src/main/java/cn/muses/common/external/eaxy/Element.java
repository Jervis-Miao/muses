...
public class Element implements Node{
    ...
    Element(QualifiedName name, Content... contents) {
        this(name, Objects.list(contents, Node.class), Objects.list(contents, Attribute.class),
                Objects.list(contents, Namespace.class));
    }

    public Element(QualifiedName name, List<Node> children, Collection<Attribute> attrs,
    ...
    private String printNamespaces(LinkedList<Namespace> printedNamespaces) {
        StringBuilder result = new StringBuilder();
        for (Namespace namespace : namespaces) {
            if (!namespace.isNamespace())
                throw new IllegalStateException(name());
            if (printedNamespaces.contains(namespace))
                continue;
