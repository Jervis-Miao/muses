...
package com.muses.common.orm.mybatis.easylist.paginator;
...
import com.muses.common.orm.mybatis.easylist.paginator.dialect.Dialect;
...
import com.muses.common.orm.mybatis.easylist.paginator.support.PropertiesHelper;
import com.muses.common.orm.mybatis.easylist.paginator.support.SQLHelp;
...
public class OffsetLimitInterceptor implements Interceptor {
	...
	public Object intercept(final Invocation invocation) throws Throwable {
		...
		if (pageBounds.getOffset() == RowBounds.NO_ROW_OFFSET && pageBounds.getLimit() == RowBounds.NO_ROW_LIMIT
				&& !pageBounds.hasWhereClause() && !pageBounds.hasOrderByClause()) {
		...
		// Where Sql
		final Object newParam = dialect.getParameterObject();
		queryArgs[MAPPED_STATEMENT_INDEX] = copyFromNewSql(ms, boundSql, dialect.getPageSQL(),
				dialect.getParameterMappings(), newParam);
		queryArgs[PARAMETER_INDEX] = newParam;
		...
		if (pageBounds.isContainsTotalCount()) {
			final Map<String, Object> countParam = dialect.getCountParameterObject();
			final MappedStatement countMs = copyFromNewSql(ms, boundSql, dialect.getCountSQL(),
					dialect.getCountParameterMappings(), countParam);
			final BoundSql countBoundSql = countMs.getBoundSql(countParam);

			Callable<Paginator> countTask = new Callable() {
				@Override
				public Object call() throws Exception {
					Integer count;
					Cache cache = countMs.getCache();
					if (cache != null && countMs.isUseCache()) {
						CacheKey cacheKey = executor.createCacheKey(
								countMs,
								countParam,
								new PageBounds(),
								copyFromBoundSql(countMs, countBoundSql, dialect.getCountSQL(),
										countBoundSql.getParameterMappings(), countBoundSql.getParameterObject()));
						count = (Integer) cache.getObject(cacheKey);
						if (count == null) {
							count = SQLHelp.getCount(countMs, countParam, countBoundSql, dialect);
