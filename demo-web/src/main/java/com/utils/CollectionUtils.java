...
public class CollectionUtils {
	 ...
	 * @return
	 */
	public static <K> Map<String, Object> list2Map(List<K> list, ObjectToMapIterface interIterface) {
		Map<String, Object> map = new HashMap<>();
		if (CollectionUtils.isNotEmpty(list)) {
			for (K k : list) {
				ObjectToMapBase base = interIterface.obj2ObjectToMapBase(k);
				if (null != base) {
					map.put(base.key, base.value);
				}
			}
		}
		return map;
	...
	public static <T> List<T> difference(Collection<T> e1, Collection<T> e2) {
		List<T> l1 = new ArrayList<T>(e1);
		List<T> l2 = new ArrayList<T>(e2);
		l1.removeAll(l2);
		return l1;
	}
